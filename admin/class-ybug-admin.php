...
class Ybug_Admin {
    ...
    public function render_visibility_field(  ) {
        ...
        $options = get_option( 'ybug' );
        $options['visibility'] = @$options['visibility'] ?: 'frontend';
        ?>
        <p>
            <label>
                <input name="ybug[visibility]" type="radio" value="frontend" class="tog" <?php checked( 'frontend' === $options['visibility'] ); ?> />
                <?php echo __( 'Show on the Website', 'ybug' ); ?>
            </label>
        </p>
        <p>
            <label>
                <input name="ybug[visibility]" type="radio" value="admin" class="tog" <?php checked( 'admin' === $options['visibility'] ); ?> />
                <?php echo __( 'Show in the Admin Area', 'ybug' ); ?>
            </label>
        </p>
        <p>
            <label>
                <input name="ybug[visibility]" type="radio" value="both" class="tog" <?php checked( 'both' === $options['visibility'] ); ?> />
                <?php echo __( 'Show on both (Website &amp; Admin Area)', 'ybug' ); ?>
            </label>
        </p>
        <?php
...
If you haven't done that yet, you can <a href='https://ybug.io/dashboard' target='_blank'>create a new project now</a>. It should only take a minute.</p>";
    ...
    {
        $this->enqueue_ybug_snippet();
	}

    /**
     * Render JS snippet for the admin area
     *
     * @since    1.2.0
     */
    public function enqueue_ybug_snippet()
    {
        $options = get_option( 'ybug' );
        ...
        // Show only if admin visibility is enabled
        if (!@$options['visibility'] || $options['visibility'] === 'frontend') {
            return;
        }
...
<!-- Ybug code end -->";
        ...
        wp_register_script('ybug-admin-snippet', '');
        wp_enqueue_script('ybug-admin-snippet');
        wp_add_inline_script('ybug-admin-snippet', $snippet);
    }
